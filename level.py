...
def level():
	spi = MCP3204(0)
	a1 = spi.read(1)
	a1_status = check_level(a1)
	a2 = spi.read(2)
	a2_status = check_level(a2)
	#a3 = spi.read(3)
	#a3_status = check_level(a3)           
	return a1_status,a2_status
def check_level(a):
	if a >= up_threshold:
	    return "ON"
	else:
	    return "OFF"
def response(sensor_1,sensor_2):
    if sensor_1 and sensor_2:
        return "full tank"
    if sensor_1 == False and sensor_2:
        return "error level sensor"
    if sensor_1 and sensor_2 == False:
        return "average tank"
    if sensor_1== False and sensor_2 == False:
        return "empty "    
def log(message):
    if DEBUG:
	print str(message)
